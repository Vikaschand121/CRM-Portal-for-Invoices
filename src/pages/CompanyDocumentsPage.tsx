    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [docs, company] = await Promise.all([
          documentsService.getDocumentsByCompany(parseInt(companyId)),
  ...
  if (error) {
    return (
      <Container maxWidth="lg" sx={{ mt: 6 }}>
        ...
        <Button startIcon={<ArrowBack />} onClick={() => navigate(`/companies/${companyId}`)}>
          Back to Company
        </Button>
      </Container>
    );
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2 }}>
                    <Button
                      ...
                      onClick={() => {
                        if (group.subType) {
                          navigate(`/companies/${companyId}/documents/subtype/${encodeURIComponent(group.subType)}`);
                        } else {
                          navigate(`/companies/${companyId}/documents/type/${encodeURIComponent(group.type)}`);
                        }
                      }}
                      variant="outlined"
                      size="small"
